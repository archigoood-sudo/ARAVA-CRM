...
const desktopApi: AravaDesktopApi = {
  ...
  trials: {
    cancel: (token, id, input) => invoke(IPC_CHANNELS.trialCancel, token, id, input),
    ...
    setOutcome: (token, id, input) => invoke(IPC_CHANNELS.trialOutcome, token, id, input),
  ...
  integration: {
    ...
    diagnosePermanentFailureRecovery: (token) =>
  ...
  paymentOperations: {
    ...
    recoverPendingSales: (token) => invoke(IPC_CHANNELS.paymentOperationRecoverSales, token),
