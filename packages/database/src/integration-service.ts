    ...
    switch (change.entityType) {
      ...
      case 'LESSON': {
        ...
        const data = {
          ...
          groupId: requiredString(payload, 'groupId'),
          makeupForLessonId: nullableString(payload.makeupForLessonId),
          makeupRequired: booleanValue(payload.makeupRequired, false),
          notes: nullableString(payload.notes),
          originalEndsAt: optionalDate(payload.originalEndsAt),
          originalStartsAt: optionalDate(payload.originalStartsAt),
          payoutCategory: enumValue(
            payload.payoutCategory,
            ['REGULAR_ATTENDANCE', 'MAKEUP', 'PROMOTIONAL_FREE', 'PERSONAL_LESSON'] as const,
            'REGULAR_ATTENDANCE',
          ),
          room: nullableString(payload.room),
          roomId: nullableString(payload.roomId),
          rescheduledAt: optionalDate(payload.rescheduledAt),
          rescheduledFromCoachId: nullableString(payload.rescheduledFromCoachId),
          rescheduledFromRoomId: nullableString(payload.rescheduledFromRoomId),
      ...
      case 'SUBSCRIPTION': {
        ...
        const data = {
          ...
          sequenceAfterSubscriptionId: nullableString(payload.sequenceAfterSubscriptionId),
          startsAt: requiredDate(payload, 'startsAt'),
          status: enumValue(
            payload.status,
      ...
      case 'SUBSCRIPTION_LEDGER': {
        ...
        const data = {
          ...
          periodEndsAt: optionalDate(payload.periodEndsAt),
          periodStartsAt: optionalDate(payload.periodStartsAt),
      ...
      case 'LESSON': {
        ...
        return row
          ? {
              ...
              groupId: row.groupId,
              id: row.id,
              makeupForLessonId: row.makeupForLessonId,
              makeupRequired: row.makeupRequired,
              originalEndsAt: iso(row.originalEndsAt),
              originalStartsAt: iso(row.originalStartsAt),
              roomId: row.roomId,
              room: row.room,
              notes: row.notes,
              payoutCategory: row.payoutCategory,
              rescheduledAt: iso(row.rescheduledAt),
              rescheduledFromCoachId: row.rescheduledFromCoachId,
              rescheduledFromRoomId: row.rescheduledFromRoomId,
      ...
      case 'SUBSCRIPTION': {
        ...
        return row
          ? {
              ...
              sequenceAfterSubscriptionId: row.sequenceAfterSubscriptionId,
              startsAt: row.startsAt.toISOString(),
              status: row.status,
              studentId: row.studentId,
      ...
      case 'SUBSCRIPTION_LEDGER': {
        ...
        return row
          ? {
              ...
              lessonId: row.lessonId,
              periodEndsAt: iso(row.periodEndsAt),
              periodStartsAt: iso(row.periodStartsAt),
