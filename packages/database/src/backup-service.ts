...
export class BackupService {
  ...
  async restoreBackup(
    ...
    const stagePath = join(dirname(this.databasePath), `.arava-restore-${randomUUID()}.db`);
    ...
    try {
      await copyFile(selected.path, stagePath, constants.COPYFILE_EXCL);
      const stagedDatabase = createDatabaseClient(`${toSqliteUrl(stagePath)}?connection_limit=1`);
      try {
      ...
      await rename(stagePath, this.databasePath);
  ...
  private async createSnapshot(path: string, type: BackupType): Promise<BackupEntry> {
    ...
    try {
      ...
      await this.database.$executeRawUnsafe(`VACUUM INTO ${sqliteString(path)}`);
  ...
  private async validatePath(
    ...
    try {
      ...
      const entry = await this.entryForPath(path, {
        createdAt:
          (await this.readMetadata(path))?.createdAt ?? (await stat(path)).mtime.toISOString(),
        integrity: 'VALID',
        type,
      });
      if (updateMetadata)
      ...
      return {
        ...
        canRestore: true,
        integrity: 'VALID',
        message: 'Резервная копия исправна и может быть восстановлена.',
